...
ruoyi-ui/src/
├── views/
│   ├── index.vue              # 首页数据看板（v3.4新增）
│   └── dashboard/
│       └── useDashboardData.ts # 看板数据聚合 composable（v3.4新增）
...
- **首页看板**：统计卡片（今日订单/运输中/本月营收/待开票）、月度订单金额趋势图（可切换月份）、订单状态分布饼图、客户营收排名Top5、订单查询（按月/日期区间搜索、客户筛选、金额合计）、发票/回单概览（v3.4新增）
